...
For local setup and the complete release build, use the repository wrapper:
...
4. Build the complete production output:
...
Before merging a substantive change, run the same guarded build used for a local release:

```bash
./bin/portfolio build
```

This renders and compiles the CV, validates its PDF, runs formatting and source-contract checks, cleans stale Jekyll output, and creates a production site. For JavaScript changes, also verify publication full-name filtering, the Miscellaneous year range filter, and the sticky sidebars in a browser.
